class TeamsController < ApplicationController
  ...
  # GET /teams
  def index
    log_controller_call "View all teams"
  ...
  # GET /teams/new
  def new
    log_controller_call "View page to create new team"
  ...
  def create
    ...
    if name.strip.length > 0  && !provider_ids.blank?
      ...
      log_controller_call "Create team"
      current_user.teams << @team
      current_user.save!
      redirect_to @team
    else
      log_controller_call "Unable to create team with parameters"
  ...
  def create_default
    if current_user.practice
      log_controller_call "Create default team"
  ...
  def update
    ...
    if name.strip.length > 0  && !provider_ids.blank?
      ...
      @team.save!
      log_controller_call "Updated team"
    else
      log_controller_call "Unable to update team with parameters"
    end
  ...
  # GET /teams/1/edit
  def edit
    log_controller_call "View page to edit team"
    if current_user.admin? || APP_CONFIG['use_opml_structure']
      @providers = Provider.all
    else
  ...
  def destroy
    ...
    @team.destroy
    log_controller_call "Deleted team"
